...
if buildVersion.version_year >= 2025:
	...
	from ...sonic._sonic import SonicStream, initialize as sonicInitialize
...
def initialize():
	...
	sampleRate = 22050
	player = nvwave.WavePlayer(1, sampleRate, 16, outputDevice=config.conf["audio"]["outputDevice"] if buildVersion.version_year >= 2025 else config.conf["speech"]["outputDevice"])
