from .piper_neural_voices import SynthDriver as PiperManager
from . import Voice
...
class PiperVoice(Voice):
	...
	def variants(self):
		return [{
			"id": id,
			"name": id,
		} for id, name in self.core._getAvailableVariants().items()]
	...
	def voices(cls):
		if not cls.core:
			return []
		return cls.core.availableVoicesForWV
